...
(defclass upnp-service ()
  ((actions :initarg :actions :reader service-actions)
   (variables :initarg :variables :reader service-variables)
   (scpd-url :initarg :scpd-url :reader service-scpd-url)
   (control-url :initarg :control-url :reader service-control-url)
   (type :initarg :service-type :reader service-type)))

(defvar *upnp-services* nil)
(defvar *upnp-location* nil)



(defun gen-service-desc (device-info services)
  (with-output-to-string (s)
    (format s "<?xml version=\"1.0\" encoding=\"utf-8\" ?>~%")
    (format s "<root xmlns=\"urn:schemas-upnp-org:device-1-0\">~%")
    (format s "  <specVersion>~%")
    (format s "  <major>1</major>~%")
    (format s "  <minor>0</minor>~%")
    (format s "  </specVersion>~%")
    (format s "  <device>~%")

    (dolist (info device-info)
      (format s "    <~A>~A</~A>~%" (car info) (cdr info) (car info)))

    (format s "  <serviceList>~%")
    (dolist (svc services)
      (format s "      <service>~%")
      (format s "        <serviceType>~A</serviceType>~%" (service-type svc))
      (format s "        <serviceId>~A</serviceId>~%" (service-id svc))
      (format s "        <controlURL>~A</controlURL>~%" (service-control-url svc))
      (format s "        <eventSubURL>~A</eventSubURL>~%" (service-event-url svc))
      (format s "        <SCPDURL>~A</SCPDURL>~%" (service-scpd-url svc))
      (format s "      </service>~%"))
    (format s "  </serviceList>~%")
...
(defclass upnp-acceptor (hunchentoot:acceptor)
  ()
  (:default-initargs :address nil))

(defun acceptor-handler (a request)
  (setf (hunchentoot:content-type*) "text/xml")  
  (let ((path (hunchentoot:request-pathname request))
	(method (hunchentoot:request-method request)))
    ;; things we need to handle
    ;; 1. GET device description (SSDP location URL) - GET-SERVICE-LIST 
    ;; 2. GET service description (SCPD URL) - GET-SERVICE-ACTIONS 
    ;; 3. POST call rpc, control url - CALL-SERVICE-ACTION 
    (cond
      ((eq method :get)
       ;; look for device description
       (if (string-equal path (acceptor-desc-url a))
	   (gen-desc a)
	   (dolist (svc (acceptor-services a))
	     (when (string-equal path (service-desc-url svc))
	       (return-from acceptor-handler (gen-service-desc svc))))))
      ((eq method :post)
       ;; look for a service handler
       (dolist (svc (acceptor-services a))
	 (when (string-equal (service-control-url svc) path)
	   (let ((post-data (hunchentoot:raw-post-data request)))
	     (return-from acceptor-handler (process-service-action post-data))))))
      (t
       ))))


(defmethod hunchentoot:acceptor-dispatch-request ((a upnp-acceptor) request)
  (acceptor-handler a request))

;; start the server
(defun make-upnp-acceptor (port)  
  (make-instance 'upnp-acceptor :port port))

(defvar *upnp-acceptor* nil)

(defun start-upnp-server (port)
  (when *upnp-acceptor* (error "Server already running"))
  (setf *upnp-acceptor* (make-upnp-acceptor port))
  (hunchentoot:start *upnp-acceptor*))

(defun stop-upnp-server ()
  "Stop the xml/rpc server"
  (when *upnp-acceptor*    
    (hunchentoot:stop *upnp-acceptor*)
    (setf *upnp-acceptor* nil)))
